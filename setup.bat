...
echo.
echo   %CYAN%[1]%NC% %BOLD%Start Open_Intel%NC%  %DIM%-- use existing config%NC%
...
set /p ADMIN_EMAIL=  Admin email [admin@open_intel.tech]:
if not defined ADMIN_EMAIL set ADMIN_EMAIL=admin@open_intel.tech
...
if defined HASH (
    docker compose -f infra/docker-compose.yml exec -T postgres psql -U voidaccess -d voidaccess -c "UPDATE users SET hashed_password='!HASH!', must_reset_password=false, email='!ADMIN_EMAIL!' WHERE email='admin@open_intel.tech' OR email='!ADMIN_EMAIL!';" >nul 2>&1
...
echo %GREEN%  +===================================+%NC%
echo %GREEN%  ^|                                   ^|%NC%
echo %GREEN%  ^|   [OK]  Open_Intel is ready       ^|%NC%
echo %GREEN%  ^|                                   ^|%NC%
