#!/usr/bin/env bash
# Open_Intel Interactive Setup Wizard
...
if [ "$HAS_ENV" = "true" ] || [ "$HAS_VOLUMES" = "true" ] || [ "$HAS_CONTAINERS" = "true" ]; then
    ...
    printf "  ${CYAN}[1]${NC} ${BOLD}Start Open_Intel${NC}  ${DIM}— use existing config${NC}\n"
...
if [ "$DOCKER_OK" = false ]; then
    print_fail "Docker is required to run Open_Intel."
    printf "\n"
...
try:
    urllib.request.urlretrieve(
        ...
        '/tmp/open_intel_mitre_attack.json'
    )
...
printf "\n"
printf "  ${CYAN}▸${NC}  Start Open_Intel now? "
printf "[${BOLD}Y${NC}/n]: "
...
ADMIN_EMAIL="admin@open_intel.tech"
ADMIN_PASS=""

if [ -t 0 ]; then
    prompt "Admin email [admin@open_intel.tech]: "
    read -r _email_input || _email_input=""
    ADMIN_EMAIL="${_email_input:-admin@open_intel.tech}"
    ...
    if [ -n "$HASH" ]; then
        $COMPOSE_CMD \
            ...
            "UPDATE users SET
             ...
             email='$ADMIN_EMAIL'
             WHERE email='admin@open_intel.tech'
