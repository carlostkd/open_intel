...
session file ("open_intel_telegram.session" in the working directory).
...
async def fetch_telegram_messages(
    ...
    try:
        # "open_intel_telegram" = session file name; StringSession("") = fresh in-memory
        # For persistent auth: use "open_intel_telegram" (creates open_intel_telegram.session)
        async with TelegramClient("open_intel_telegram", api_id, TELEGRAM_API_HASH) as client:
